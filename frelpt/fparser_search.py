...
class Searcher(object):
    ...
    def search_Call_Stmt(self, node, ids, rtypes=None):
        ...
        self._search(node.subnodes[0], ids, rtypes=res_procedure_designator)
        self._search(node.subnodes[1], ids, rtypes=res_actual_arg_spec)

    def search_Dimension_Attr_Spec(self, node, ids, rtypes=None):
        """
        <dimension-attr-spec> = DIMENSION ( <array-spec> )
        """
        self._search(node.subnodes[1], ids, rtypes=res_array_spec)

    def search_Explicit_Shape_Spec(self, node, ids, rtypes=None):
        """
        <explicit-shape-spec> = [ <lower-bound> : ] <upper-bound>
        """
        self._search(node.subnodes[0], ids, rtypes=res_lower_bound)
        self._search(node.subnodes[1], ids, rtypes=res_upper_bound)

    def search_Int_Literal_Constant(self, node, ids, rtypes=None):

        self._search(node.subnodes[1], ids, rtypes=res_kind_param)
    ...
    def search_Intrinsic_Type_Spec(self, node, ids, rtypes=None):
        ...
        if i_type == "CHARACTER":
            self._search(node.subnodes[1], ids, rtypes=res_char_selector)
        else: 
            self._search(node.subnodes[1], ids, rtypes=res_kind_selector)
    ...
    def search_Loop_Control(self, node, ids, rtypes=None):
        ...
        if scalar_logical_expr is not None:
            self._search(scalar_logical_expr, ids, rtypes=res_scalar_logical_expr)
        elif counter_expr[0] is not None and counter_expr[1] is not None:
            self._search(counter_expr[0], ids, rtypes=res_scalar_int_expr)
            self._search(counter_expr[1], ids, rtypes=res_scalar_int_expr)
    ...
    def search_Nonlabel_Do_Stmt(self, node, ids, rtypes=None):
        ...
        self._search(node.subnodes[1], ids, rtypes=res_loop_control)
    ...
    def search_Part_Ref(self, node, ids, rtypes=None):
        ...
        self._search(node.subnodes[1], ids, rtypes=res_section_subscript)
    ...
    def search_Subscript_Triplet(self, node, ids, rtypes=None):
        ...
        self._search(lhs_obj, ids, rtypes=res_subscript)
        self._search(rhs_obj, ids, rtypes=res_subscript)
        self._search(stride_obj, ids, rtypes=res_stride)
